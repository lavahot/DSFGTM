...
const int NUM_CPUS = 4;
  ...
  Processor::Processor(){};
  void Processor::init(Clock*clock_,StatModule*stat_mod_)
  //Disperse clock to CPU
  {
   ...
   {
    cpu_p.first = i;
    cpu_p.second = new CPU1(clock_,stat_mod_);
    d_cpus.Push(cpu_p);
   }
   clock = clock_;
   stat_mod = stat_mod_;
  ...
  //loop through cpu's for the second
  {
   ...
   for(int i=0;i<NUM_CPUS;i++)
   {
    while( !((d_cpus[i].second)->IsFull())&& !d_waiting_list.IsEmpty() )
    {
     ...
     //add to max jobs
     (d_cpu[i].second)->enQ(job_p);
    }

    //run cpu for second
    (d_cpu[i].second)->process(clock->getTime());
   }
  }
